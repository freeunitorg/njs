    ...
    njs_index_t unused, njs_value_t *retval)
{
    ...
    /*
     * Valid only in js_access phase.  From js_content or js_filter,
     * NGX_DECLINED becomes a 500 response via ngx_http_finalize_request.
     */
...
ngx_http_js_subrequest_done(ngx_http_request_t *r, void *data, ngx_int_t rc)
{
    ...
    if (rc == NGX_ERROR) {
        ngx_http_js_event_finalize(r->parent, NGX_ERROR);
        return NGX_OK;
    }
    ...
    int argc, JSValueConst *argv)
{
    ...
    ctx = ngx_http_get_module_ctx(r, ngx_http_js_module);
    if (ctx == NULL) {
        return JS_ThrowInternalError(cx, "failed to get request context");
    }
